-- ============================================================
-- 2026-05-29 — Mundial 2026: bracket completo pre-Mundial
-- ============================================================
-- Cambio de mecánica de Polla:
-- - Antes: predicciones cerraban 1h antes de cada partido individual
-- - Ahora: TODAS las predicciones (72 marcadores grupos + 32 bracket picks KO)
--   cierran al enrollment_deadline del pool (11-jun-2026 11:00 PA).
--
-- Para los 32 partidos de eliminatoria, el user predice SOLO el equipo
-- ganador (bracket pick, sin marcador). Scoring: 5 pts × multiplier_por_fase.
-- Bonus picks pre-Mundial (campeón/sub/3°/goleador/MVP) mantienen su deadline.
-- ────────────────────────────────────────────────────────────

alter table public.wc_predictions
  add column if not exists pred_winner_team_id uuid references public.wc_teams(id) on delete restrict;

alter table public.wc_predictions
  alter column pred_score_home drop not null;
alter table public.wc_predictions
  alter column pred_score_away drop not null;

alter table public.wc_predictions
  drop constraint if exists chk_pred_has_content;
alter table public.wc_predictions
  add constraint chk_pred_has_content check (
    (pred_score_home is not null and pred_score_away is not null)
    or pred_winner_team_id is not null
  );

create index if not exists idx_wc_predictions_winner_team
  on public.wc_predictions(pred_winner_team_id)
  where pred_winner_team_id is not null;

-- RPC actualizado: deadline = enrollment_deadline; grupos requieren marcador, KO requieren team
...
begin
  ...
  if v_match.phase = 'group' then
    ...
    insert into public.wc_predictions (enrollment_id, match_id, pred_score_home, pred_score_away, pred_winner_team_id)
      values (v_enrollment.id, p_match_id, null, null, p_pred_winner_team_id)
      on conflict (enrollment_id, match_id) do update set
        pred_score_home = null,
        pred_score_away = null,
...
-- wc_resolve_polla_match: scoring por fase
  ...
  loop
    ...
    if v_match.phase = 'group' then
      ...
      if v_pred.pred_winner_team_id is not null
        ...
        v_hit_level := 'winner'; v_base_points := 5;
      end if;
    end if;
