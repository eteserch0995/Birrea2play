...
export function buildEventShareMessage(event, opts = {}) {
  ...
  const isMixto = event.genero === 'Mixto' && event.cupos_hombres && event.cupos_mujeres;
  ...
  if (isMixto) {
    ...
    const invitados = jugadores.filter(j => !j.genero).map(j => j.nombre);
    lines.push('');
    lines.push(`🔵 Hombres (${hombres.length}/${event.cupos_hombres}):`);
    lines.push(...buildNumberedList(hombres, event.cupos_hombres));
    lines.push('');
    lines.push(`🟣 Mujeres (${mujeres.length}/${event.cupos_mujeres}):`);
    lines.push(...buildNumberedList(mujeres, event.cupos_mujeres));
    if (invitados.length > 0) {
      lines.push('');
      lines.push(`👥 Invitados (${invitados.length}):`);
      invitados.forEach((nombre, i) => lines.push(`${i + 1}. ✓ ${nombre}`));
    }
