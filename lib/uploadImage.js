...
async function uriToBlob(uri) {
  ...
  try {
    ...
    throw new Error(`No se pudo leer la imagen (fetch falló: ${e.message}). El URL puede haber expirado — vuelve a seleccionar la imagen.`);
  }
...
export async function uploadImage(bucket, path, source) {
  ...
  const { contentType } = inferExtAndType(sourceUri ?? '');
