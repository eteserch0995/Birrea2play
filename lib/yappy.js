 ...
 *
 * Integración actual: Botón de Pago Yappy V2
 *   API: https://apipagosbg.bgeneral.cloud  (globalmente accesible, sin restricción de DNS)
 *
 * El flujo completo va server-side vía Edge Functions:
 *   - yappy-boton: valida comercio + crea orden → retorna { transactionId, token, documentName }
 *   - yappy-ipn:   recibe el webhook de Yappy y acredita el wallet
 *   - yappy-confirm: acredita el wallet desde el cliente (fallback si IPN falla)
 */
...
// ─── Pago Yappy — flujo web redirect ─────────────────────────────────────────

/**
 * Construye la URL de la página de recarga de Yappy pasando el JWT del usuario.
 * La página web maneja el llamado a la Edge Function y la redirección a Yappy.
 */
export async function iniciarPagoWebYappy(amount) {
  let session;
  try {
    const { data } = await supabase.auth.getSession();
    session = data?.session;
  ...
  return `https://birrea2play.com/recarga.html?${q.toString()}`;
}
