 ...
 */
export async function iniciarBotonYappy({ phone, amount, tipo = 'recarga', event_id = null, guest_id = null, wc_enrollment_id = null, credito_monto = null, cancha_reserva_id = null }) {
  let session;
  try {
    const { data } = await supabase.auth.getSession();
    session = data?.session;
  ...
  const res = await fetch(`${FUNCTIONS_URL}/yappy-boton`, {
    ...
    body: JSON.stringify({
      ...
      guest_id,
      wc_enrollment_id,
      credito_monto:      credito_monto ? Number(credito_monto) : null,
      cancha_reserva_id:  cancha_reserva_id ?? null,
    }),
  });
...
export function pollBotonOrder({ orderId, onProgress }) {
  ...
  let intervalId = null;
  let lastStatus = null;
  // Referencia al reject de la Promise para que cancel() pueda rechazarla.
  // Sin esto, el caller hace `await promise` y queda colgado cuando se cancela.
  ...
  return {
    ...
    cancel: () => {
      ...
      cancelled = true;
      cleanup();
      // Rechazar la promise para que el `await promise` del caller entre al
      // catch y limpie cualquier guest/registro pendiente. Antes esto faltaba
      // y dejaba registros huerfanos en pending_payment para siempre.
      if (externalReject) externalReject(new Error('cancelled'));
    },
  };
}
...
export function pollForYappyPayment({ userId, amount, reference, onProgress }) {
  ...
  return {
    ...
    cancel: () => {
      ...
      cancelled = true;
      cleanup();
      // Rechazar la promise para que el caller limpie estados pendientes.
      if (externalReject) externalReject(new Error('cancelled'));
    },
  };
}
