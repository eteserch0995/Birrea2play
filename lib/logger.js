...
// Los errores de supabase-js / postgrest NO son `instanceof Error` — son objetos
// planos `{ code, message, details, hint }`. Antes mandábamos `null` y el
// `error_message` quedaba vacío en client_logs (debugging a ciegas).
// Esta helper preserva message + stack para cualquier shape de error.
function toRemoteError(technical) {
  if (!technical) return null;
  if (technical instanceof Error) return technical;
  if (typeof technical === 'object') {
    const msg = technical.message ?? technical.details ?? technical.hint ?? JSON.stringify(technical);
    const wrapped = new Error(msg);
    if (technical.code)    wrapped.code    = technical.code;
    if (technical.details) wrapped.details = technical.details;
    if (technical.hint)    wrapped.hint    = technical.hint;
    return wrapped;
  }
  return new Error(String(technical));
}
...
export function logError({ screen, action, userId, eventId, technical, userMessage, extra }) {
  ...
  try {
    getRemoteLog()({
      ...
      data: { userId, code: technical?.code, status: technical?.status, details: technical?.details ?? null, hint: technical?.hint ?? null, userMessage, extra },
      error: toRemoteError(technical),
    });
  } catch (_) {}
}
...
export function logWarn({ screen, action, userId, eventId, technical, extra }) {
  ...
  try {
    getRemoteLog()({
      ...
      level: 'warn',
      eventId: eventId ?? null,
      data: { userId, code: technical?.code, details: technical?.details ?? null, extra },
      error: toRemoteError(technical),
    });
  } catch (_) {}
}
