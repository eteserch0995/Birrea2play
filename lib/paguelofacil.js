 ...
 */
export async function iniciarPagoTarjeta({ userId, amount, descripcion, tipo = 'recarga_tarjeta', credito_monto = null, wc_enrollment_id = null }) {
  const headers = await authHeaders();
  const res     = await fetch(`${FUNCTIONS_URL}/pf-create-link`, {
    method:  'POST',
    headers,
    body:    JSON.stringify({ userId, amount, descripcion, tipo, credito_monto, wc_enrollment_id }),
  });
